...
fun createMatrixClientStartedModule() = module { single<MatrixClientStarted> { MatrixClientStarted() } }

fun createClockModule() = module { single<Clock> { Clock.System } }
...
fun createTrixnityDefaultModuleFactories(): List<ModuleFactory> =
    listOf(
 ...
 * You can encrypt and decrypt events by iterating through all [RoomEventEncryptionService] (can be received via
 * [MatrixClient.roomEventEncryptionServices]) and use the first non-null result. For sending events asynchronously you
 * can still use the outbox.
 */
fun createTrixnityBotModuleFactories(): List<ModuleFactory> =
    listOf(
        ::createMatrixClientStartedModule,
        ::createClockModule,
        ::createServerModule,
