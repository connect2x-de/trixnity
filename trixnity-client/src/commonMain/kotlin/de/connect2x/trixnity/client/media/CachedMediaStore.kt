...
abstract class CachedMediaStore(coroutineScope: CoroutineScope, config: MatrixClientConfiguration, clock: Clock) :
    MediaStore {
    ...
    private val mediaCacheRemoverJobExecutingIndex =
        RemoverJobExecutingIndex("media", mediaCache, clock, config.cacheExpireDurations.media).also {
            mediaCache.indexes.update { indexes -> indexes + it }
        }
            ...
            if (cacheDisabled) {
                ...
                cacheValue.value.valueOrNull().also { if (it != null) log.trace { "cache hit for $uri" } }
                    ?: mediaCacheMutex.withLock(uri) {
                        cacheValue.value.valueOrNull().also { if (it != null) log.trace { "deep cache hit for $uri" } }
                            ?: run {
