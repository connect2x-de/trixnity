...
abstract class StoreTransactionManager : TransactionManager<StoreReadTransaction, StoreWriteTransaction> {
    protected abstract suspend fun <T> repositoryReadTransaction(
    ...
    override suspend fun <T> readTransaction(
        block: suspend StoreReadTransaction.() -> T
    ): T = repositoryReadTransaction(block)

    override suspend fun <T> writeTransaction(
        block: suspend StoreWriteTransaction.() -> T
    ): T =
