...
class RoomListHandlerTest : TrixnityBaseTest() {
    ...
    private val createEvent =
        StateEvent(CreateEventContent(), EventId("event1"), UserId("user1", "localhost"), roomId, 0, stateKey = "")
    ...
    fun `updateRoomList » lastEventId » must not update `() = runTest {
        tm.writeTransaction { roomStore.update(roomId) { simpleRoom.copy(lastEventId = EventId("old")) } }
        cut.updateRoomList(
            SyncEvents(
                Sync.Response(
                    room =
    ...
    fun `updateRoomList » name » keep when no change`() = runTest {
        ...
        cut.updateRoomList(
            SyncEvents(
                Sync.Response(nextBatch = "", room = Sync.Response.Rooms(join = roomMapOf(roomId to JoinedRoom()))),
                emptyList(),
            )
        )
    ...
    @Test
    fun `isDirect » set the room to direct == 'true' when a DirectEventContent is found for the room`() = runTest {
        tm.writeTransaction { roomStore.update(roomId) { Room(roomId, isDirect = false) } }
        val eventContent =
            DirectEventContent(
                mappings = mapOf(UserId("user1", "localhost") to setOf(RoomId("!room2:localhost"), roomId))
            )
    ...
    @Test
    fun `isDirect » membership is LEAVE or BAN » don't change isDirect`() = runTest {
        ...
        val eventContent = DirectEventContent(mappings = mapOf())
    ...
    @Test
    fun `isDirect » set the room to direct == 'false' when no DirectEventContent is found for the room`() = runTest {
        ...
        val eventContent = DirectEventContent(mappings = mapOf(UserId("user1", "localhost") to setOf(room2)))
        roomStore.getAll().first { it.size == 2 }
    ...
    fun `avatarUrl » room is direct » set the avatar URL to a member's avatar URL`() = runTest {
        ...
        val eventContent = DirectEventContent(mappings = mapOf(alice to setOf(roomId, RoomId("!room2:localhost"))))
    ...
    fun `avatarUrl » membership is LEAVE or BAN » don't change avatar'`() = runTest {
        ...
        tm.writeTransaction {
            roomStore.update(roomId) {
                Room(roomId, avatarUrl = "mxc://localhost/abcdef", membership = Membership.LEAVE)
            }
            roomStore.update(roomId2) {
                Room(roomId2, avatarUrl = "mxc://localhost/abcdef", membership = Membership.BAN)
            }
        }
        val eventContent = DirectEventContent(mappings = mapOf())
    ...
    fun `avatarUrl » room is direct » update the room's avatar URL`() = runTest {
        tm.writeTransaction {
            ...
            globalAccountDataStore.save(
                ClientEvent.GlobalAccountDataEvent(DirectEventContent(mappings = mapOf(alice to setOf(roomId))))
            )
        }
        val event1 =
            StateEvent(
                MemberEventContent(avatarUrl = "mxc://localhost/right", membership = Membership.JOIN),
                EventId("1"),
                alice,
                roomId,
                0L,
        ...
        val event2 =
            StateEvent(
                MemberEventContent(avatarUrl = "mxc://localhost/wrong", membership = Membership.JOIN),
                EventId("2"),
                alice,
                roomId,
                0L,
    ...
    fun `avatarUrl » room is direct » set the avatar URL to a member of a direct room when the new avatar URL is empty`() =
        runTest {
            tm.writeTransaction {
                ...
                roomStateStore.save(
                    StateEvent(
                        MemberEventContent(avatarUrl = "mxc://localhost/123456", membership = Membership.JOIN),
                        EventId("1"),
                        bob,
                        roomId,
                        0L,
            ...
            val event = StateEvent(AvatarEventContent(""), EventId("1"), bob, roomId, 0L, stateKey = bob.full)
    ...
    fun `avatarUrl » room id not direct » do nothing on member event`() = runTest {
        tm.writeTransaction { roomStore.update(roomId) { Room(roomId) } }
        val event =
            StateEvent(
                MemberEventContent(avatarUrl = "mxc://localhost/123456", membership = Membership.JOIN),
                EventId("1"),
    ...
    fun `avatarUrl » room id not direct » set the avatar URL for normal rooms`() = runTest {
        tm.writeTransaction { roomStore.update(roomId) { Room(roomId, avatarUrl = "mxc://localhost/abcdef") } }
        val event =
            StateEvent(AvatarEventContent("mxc://localhost/123456"), EventId("1"), bob, roomId, 0L, stateKey = bob.full)
    ...
    fun `displayName » do nothing when room summary did not change at all`() = runTest {
        val roomSummary =
            JoinedRoom.RoomSummary(heroes = listOf(user1, user2), joinedMemberCount = 1, invitedMemberCount = 2)
        val roomBefore = simpleRoom.copy(name = RoomDisplayName(explicitName = "bla", summary = roomSummary))
        tm.writeTransaction { roomStore.update(roomId) { roomBefore } }
    ...
    fun `displayName » name event found in store » set explicit name`() = runTest {
        tm.writeTransaction { roomStateStore.save(nameEvent(1, user1, "explicit")) }
    ...
    fun `displayName » name alias event found in store » set explicit name`() = runTest {
        tm.writeTransaction { roomStateStore.save(canonicalAliasEvent(1, user1, RoomAliasId("#explicit:room"))) }
        cut.calculateDisplayName(roomId, summary = JoinedRoom.RoomSummary(), membership = Membership.JOIN)
            .shouldNotBeNull { explicitName shouldBe "#explicit:room" }
    }
    ...
    fun `displayName » no name event found » joined and invited count le 5 » other users count is 0`() = runTest {
        ...
        cut.calculateDisplayName(roomId, summary = summary, membership = Membership.JOIN) shouldBe
            RoomDisplayName(
                explicitName = null,
                heroes = listOf(user1, user2, user3, user4, user5),
                otherUsersCount = 0,
    ...
    fun `displayName » no name event found » joined and invited count is 6 » we are part of server heroes and thus other users count is 1 and heroes does not include us`() =
        runTest {
            ...
            cut.calculateDisplayName(roomId, summary = summary, membership = Membership.INVITE) shouldBe
                RoomDisplayName(
                    explicitName = null,
    ...
    @Test
    fun `displayName » no name event found » joined and invited count is 8 » other users count is 2`() = runTest {
        val summary =
            JoinedRoom.RoomSummary(
                heroes = listOf(user1, user2, user3, user4, user5),
                joinedMemberCount = 6,
    ...
    fun `displayName » no name event found » we left the room » other users count is correct`() = runTest {
        ...
        cut.calculateDisplayName(roomId, summary = summary, membership = Membership.LEAVE) shouldBe
            RoomDisplayName(
                explicitName = null,
                heroes = listOf(user1, user2, user3, user4, user5),
    ...
    fun `displayName » no name event found » no explicit heroes set » use joined members as heroes excluding us`() =
        runTest {
            ...
            val summary = JoinedRoom.RoomSummary(joinedMemberCount = 3, invitedMemberCount = 2)
            cut.calculateDisplayName(roomId, summary = summary, membership = Membership.JOIN) shouldBe
                RoomDisplayName(
                    explicitName = null,
    ...
    fun `displayName » no name event found » no explicit heroes set » no joined members » use left members as heroes`() =
        runTest {
            ...
            val summary = JoinedRoom.RoomSummary(joinedMemberCount = 1, invitedMemberCount = 0)
            cut.calculateDisplayName(roomId, summary = summary, membership = Membership.JOIN) shouldBe
                RoomDisplayName(
                    explicitName = null,
                    heroes = listOf(user1, user2, user3, user4),
    ...
    fun `displayName » no name event found » joined members is gt 1 » isEmpty is false`() = runTest {
        ...
        val summary = JoinedRoom.RoomSummary(joinedMemberCount = 1, invitedMemberCount = 1)
        cut.calculateDisplayName(roomId, summary = summary, membership = Membership.JOIN) shouldBe
            RoomDisplayName(
                explicitName = null,
                heroes = listOf(user4),
    ...
    fun `displayName » no name event found » joined members is eq 1 » summary does not set it » isEmpty is true`() =
        runTest {
            ...
            val roomSummary =
                JoinedRoom.RoomSummary(heroes = listOf(), joinedMemberCount = null, invitedMemberCount = null)
            val roomBefore =
                simpleRoom.copy(
                    name = RoomDisplayName(explicitName = null, summary = roomSummary.copy(joinedMemberCount = 0))
                )
            tm.writeTransaction { roomStore.update(roomId) { roomBefore } }
    ...
    fun `displayName » merge summary correctly`() = runTest {
        val oldSummary = JoinedRoom.RoomSummary(heroes = listOf(user2), joinedMemberCount = 4, invitedMemberCount = 1)
        val roomBefore = simpleRoom.copy(name = RoomDisplayName(summary = oldSummary))
        tm.writeTransaction { roomStore.update(roomId) { roomBefore } }
        val roomSummary =
            JoinedRoom.RoomSummary(heroes = listOf(user1), joinedMemberCount = 3, invitedMemberCount = null)
        cut.calculateDisplayName(roomId, summary = roomSummary, membership = Membership.JOIN) shouldNotBeNull
            {
                summary shouldBe
                    JoinedRoom.RoomSummary(heroes = listOf(user1), joinedMemberCount = 3, invitedMemberCount = 1)
            }
    }
    ...
    fun `deleteLeftRooms » forget rooms on leave when activated`() = runTest {
        ...
        cut.deleteLeftRooms(
            SyncEvents(
                Sync.Response(nextBatch = "", room = Sync.Response.Rooms(leave = roomMapOf(roomId to LeftRoom()))),
                emptyList(),
            )
        )
    ...
    fun `deleteLeftRooms » not forget rooms on leave when disabled`() = runTest {
        ...
        tm.writeTransaction { roomStore.update(roomId) { simpleRoom.copy(membership = Membership.LEAVE) } }
    ...
    @Test
    fun `deleteLeftRooms » do forget rooms where timeline is empty`() = runTest {
        config.deleteRooms = DeleteRooms.WhenNotJoined
        val room = simpleRoom.copy(membership = Membership.LEAVE)
        tm.writeTransaction { roomStore.update(roomId) { room } }
    ...
    @Test
    fun `deleteLeftRooms » do not forget rooms where there is a JOIN event for us`() = runTest {
        config.deleteRooms = DeleteRooms.WhenNotJoined
        val room = simpleRoom.copy(membership = Membership.LEAVE)
        tm.writeTransaction { roomStore.update(roomId) { room } }
        roomServiceMock.rooms.value = mapOf(roomId to MutableStateFlow(room))
    ...
    @Test
    fun `deleteLeftRooms » state event of other user found in room so do NOT delete`() = runTest {
        config.deleteRooms = DeleteRooms.WhenNotJoined
        val room = simpleRoom.copy(membership = Membership.LEAVE)
        tm.writeTransaction { roomStore.update(roomId) { room } }
        roomServiceMock.rooms.value = mapOf(roomId to MutableStateFlow(room))
    ...
    private fun nameEvent(i: Long, userId: UserId, name: String): StateEvent<NameEventContent> {
        return StateEvent(NameEventContent(name), EventId("\$event$i"), userId, roomId, i, stateKey = "")
    }
