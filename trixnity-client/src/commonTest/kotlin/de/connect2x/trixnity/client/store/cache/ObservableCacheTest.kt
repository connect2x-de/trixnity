...
class ObservableCacheTest : TrixnityBaseTest() {
    ...
    fun `read » read value from repository and update cache`() = runTest {
        noOpTm.writeTransaction { cacheStore.persist("key", "a new value") }
    ...
    fun `read » prefer cache`() = runTest {
        noOpTm.writeTransaction { cacheStore.persist("key", "a new value") }
        cut.get(key = "key").first() shouldBe "a new value"
        noOpTm.writeTransaction { cacheStore.persist("key", "a changed value") }
        cut.get(key = "key").first() shouldBe "a new value"
    }

    @Test
    fun `read » remove from cache when not used anymore`() = runTest {
        noOpTm.writeTransaction { cacheStore.persist("key", "old value") }

        val collectingJob = backgroundScope.launch { cut.get(key = "key").collect() }

        cut.get(key = "key").first() shouldBe "old value"
        delay(1.minutes + 1.milliseconds)
        cut.invalidate()
        noOpTm.writeTransaction { cacheStore.persist("key", "new value") }
        cut.get(key = "key").first() shouldBe "old value"
    ...
    fun `read » remove from cache when cache time expired`() = runTest {
        ...
        delay(1.minutes + 1.milliseconds)
        cut.invalidate()
        noOpTm.writeTransaction { cacheStore.persist("key", "another value") }
        cut.get(key = "key").first() shouldBe "another value"
        // we check, that the value is not removed before the time expires
        noOpTm.writeTransaction { cacheStore.persist("key", "yet another value") }
    ...
    fun `read » infinite cache enabled » never remove from cache`() = runTest {
        ...
        noOpTm.writeTransaction { cacheStore.persist("key", "aanother value") }
        cut.get(key = "key").first() shouldBe "a new value"
        delay(1.minutes + 1.milliseconds)
        cut.invalidate()
        cut.get(key = "key").first() shouldBe "a new value"
    ...
    fun `write » read value from repository and update cache`() = runTest {
        noOpTm.writeTransaction { cacheStore.persist("key", "from db") }
        tm.writeTransaction {
            cut.update(
                key = "key",
                updater = { oldValue ->
        ...
        tm.readTransaction { cacheStore.get("key") shouldBe "updated value" }
        tm.writeTransaction {
            cut.update(
                key = "key",
                updater = { oldValue ->
        ...
        tm.readTransaction { cacheStore.get("key") shouldBe "updated value 2" }
    }
    ...
    fun `write » prefer cache`() = runTest {
        ...
        tm.readTransaction { cacheStore.get("key") shouldBe "updated value" }
        noOpTm.writeTransaction { cacheStore.persist("key", "from db 2") }
        tm.writeTransaction {
            cut.update(
                key = "key",
                updater = { oldValue ->
    ...
    fun `write » not save unchanged value`() = runTest {
        tm.writeTransaction { cut.update(key = "key", updater = { "updated value" }) }
        tm.readTransaction { cacheStore.get("key") shouldBe "updated value" }
        noOpTm.writeTransaction { cacheStore.persist("key", null) }
        tm.writeTransaction { cut.update(key = "key", updater = { "updated value" }) }
        tm.readTransaction { cacheStore.get("key") shouldBe null }
    }
                ...
                val result = measureTimedValue {
                    (0..99)
                        .map { i ->
                            async {
                                measureTimedValue {
                                        tm.writeTransaction { cut.update(key = "key", updater = { "$i" }) }
                                    }
                                    .duration
                            }
                        }
                    ...
                    (0..99)
                        .map { i ->
                            this@withContext.async {
                                measureTimedValue {
                                        tm.writeTransaction { cut.update(key = "$i", updater = { "value" }) }
                                    }
                                    .duration
                            }
                        }
    ...
    fun `write » use same internal StateFlow when initial value is null`() = runTest {
        ...
        tm.writeTransaction { cut.update(key = "key", updater = { "newValue" }) }
    ...
    fun `write » fill value with set while read is active`() = runTest {
        ...
        tm.writeTransaction { cut.set("key", "value") }
    ...
    fun `write » fill value with set when cache entry present`() = runTest {
        ...
        tm.readTransaction { cacheStore.get("key") shouldBe "value" }
    }
    ...
    fun `write » skip cache when no read active`() = runTest {
        tm.writeTransaction { cut.set("key", "value") }
        noOpTm.writeTransaction { cacheStore.persist("key", "otherValue") }
    ...
    fun `write » infinite cache not enabled » reset expireDuration on use`() = runTest {
        tm.writeTransaction { cut.update(key = "key", updater = { "updated value 1" }) }
        delay(1.minutes + 1.milliseconds)
        cut.invalidate()
        tm.writeTransaction {
            cut.update(
    ...
    fun `write » removeFromCacheOnNull enabled » remove from cache when value is null set`() = runTest {
        ...
        tm.writeTransaction { cut.set("key", null) }
    ...
    fun `rollback cache entry on error`() = runTest {
        val throwingCacheStore =
            object : ObservableCacheStore<String, String> {
        ...
        shouldThrow<RuntimeException> { tm.writeTransaction { cut.update("key") { "new" } } }.message shouldBe "upsi"
        cut.get("key").first() shouldBe "old"
    }
    ...
    fun `not rollback cache entry on cancellation`() = runTest {
        ...
        cut.get("key").first() shouldBe "old"
        val job = async { tm.writeTransaction { cut.update("key") { "new" } } }
        onPersist.first { it }
        job.cancel()
        delay(100.milliseconds)
    ...
    fun `index » call onPut on cache insert`() = runTest {
        tm.writeTransaction { indexedCut.update("key") { "value" } }
    ...
    fun `index » allow remove from cache when index subscriptions gt 0 but value==null`() = runTest {
        indexedCut.index.subscriptionCount = 1
        tm.writeTransaction { indexedCut.update("key") { null } }
        delay(1.minutes + 1.milliseconds)
        indexedCut.invalidate()
