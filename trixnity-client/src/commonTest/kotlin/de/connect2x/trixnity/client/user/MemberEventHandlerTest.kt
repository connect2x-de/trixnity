...
class MemberEventHandlerTest : TrixnityBaseTest() {
    ...
    private val cut =
        UserMemberEventHandler(api = mockMatrixClientServerApiClient(), roomUserStore = roomUserStore, tm = tm)

    @Test
    fun `setRoomUser » skip when user already present`() = runTest {
        val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
        cut.apply {
    ...
    fun `setRoomUser » don't skip when user already present but based on stripped state`() = runTest {
        ...
        val event =
            user1Event.copy(content = MemberEventContent(displayName = "CHANGED!!!", membership = Membership.JOIN))
        cut.apply {
            setRoomUser(listOf(strippedEvent))
            setRoomUser(listOf(event), skipWhenAlreadyPresent = true)
        }
    ...
    fun `setRoomUser » user is member » no other user has same displayName » not change our displayName when it has not changed`() =
        runTest {
            setupNoOtherUserHasSameDisplayName()
            val event = user1Event.copy(content = MemberEventContent(displayName = "OLD", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))
    ...
    fun `setRoomUser » user is member » no other user has same displayName » set our displayName to user server when no displayName set`() =
        runTest {
            ...
            cut.setRoomUser(listOf(event))
            roomUserStore.get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "@user1:server", event)
        }
    ...
    fun `setRoomUser » user is member » no other user has same displayName » set our displayName to user server when displayName is empty`() =
        runTest {
            setupNoOtherUserHasSameDisplayName()
            val event = user1Event.copy(content = MemberEventContent(displayName = "", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))
            roomUserStore.get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "@user1:server", event)
        }
    ...
    fun `setRoomUser » user is member » one other user has same displayName » set displayName of the other and us to 'DisplayName user1 server'`() =
        runTest {
            setupUserIsMember()
            val event2 = user2Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            tm.writeTransaction { roomUserStore.update(user2, roomId) { RoomUser(roomId, user2, "U1", event2) } }
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))
            roomUserStore.apply {
                get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "U1 (@user1:server)", event)
                get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "U1 (@user2:server)", event2)
            }
        }
    ...
    @Test
    fun `setRoomUser » user is member » one other user has same displayName » find collision in same list`() = runTest {
        setupUserIsMember()
        val user2EventJoin =
        ...
        roomUserStore.apply {
            get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "U1 (@user2:server)", user2EventJoin)
            get(user3, roomId).first() shouldBe RoomUser(roomId, user3, "U1 (@user3:server)", user3EventJoin)
        }
    }
    ...
    fun `setRoomUser » user is member » two other users have same displayName » set our displayName to 'DisplayName user server'`() =
        runTest {
            ...
            val event2 = user2Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            tm.writeTransaction {
                roomUserStore.update(user2, roomId) { RoomUser(roomId, user2, "U1 (@user2:server)", event2) }
            }
            val event3 = user3Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            tm.writeTransaction {
                roomUserStore.update(user3, roomId) { RoomUser(roomId, user3, "U1 (@user3:server)", event3) }
            }
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))
            roomUserStore.apply {
                get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "U1 (@user1:server)", event)
                get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "U1 (@user2:server)", event2)
                get(user3, roomId).first() shouldBe RoomUser(roomId, user3, "U1 (@user3:server)", event3)
            }
        }
    ...
    fun `setRoomUser » user is member » one other user has same old displayName » set displayName of the other to 'DisplayName'`() =
        runTest {
            ...
            tm.writeTransaction {
                roomUserStore.update(user2, roomId) { RoomUser(roomId, user2, "OLD (@user2:server)", event2) }
            }
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))
            roomUserStore.get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "OLD", event2)
        }
    ...
    fun `setRoomUser » user is member » two other users have same old displayName » keep displayName of the others'`() =
        runTest {
            ...
            tm.writeTransaction {
                roomUserStore.update(user2, roomId) { RoomUser(roomId, user2, "OLD (@user2:server)", event2) }
                roomUserStore.update(user3, roomId) { RoomUser(roomId, user3, "OLD (@user3:server)", event3) }
            }
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            cut.setRoomUser(listOf(event))

            roomUserStore.apply {
                get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "OLD (@user2:server)", event2)
                get(user3, roomId).first() shouldBe RoomUser(roomId, user3, "OLD (@user3:server)", event3)
            }
        }
    ...
    fun `setRoomUser » user is not member anymore » one other user has same displayName » set displayName of the other to 'DisplayName'`() =
        runTest {
            ...
            val event2 = user2Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.JOIN))
            tm.writeTransaction { roomUserStore.update(user2, roomId) { RoomUser(roomId, user2, "U1", event2) } }
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.BAN))
            cut.setRoomUser(listOf(event))
            roomUserStore.apply {
                get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "U1", event)
                get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "U1", event2)
            }
        }
    ...
    fun `setRoomUser » user is not member anymore » two other users have same displayName » keep displayName of the others`() =
        runTest {
            ...
            val event = user1Event.copy(content = MemberEventContent(displayName = "U1", membership = Membership.LEAVE))
            cut.setRoomUser(listOf(event))
            roomUserStore.apply {
                get(user1, roomId).first() shouldBe RoomUser(roomId, user1, "U1 (@user1:server)", event)
                get(user2, roomId).first() shouldBe RoomUser(roomId, user2, "U1 (@user2:server)", event2)
        ...
        tm.writeTransaction {
            roomUserStore.update(user2, roomId) {
                RoomUser(
                    ...
                    user2,
                    "U2",
                    user2Event.copy(content = MemberEventContent(displayName = "U2", membership = Membership.JOIN)),
                )
            }
        }
    }
