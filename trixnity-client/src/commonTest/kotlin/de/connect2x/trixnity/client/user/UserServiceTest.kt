...
class UserServiceTest : TrixnityBaseTest() {
    ...
    private val powerLevelsEvent =
        powerLevelsEvent(
            PowerLevelsEventContent(
                users = mapOf(me to 55, alice to 55),
                events = mapOf(EventType(PowerLevelsEventContent::class, "m.room.power_levels") to 54),
                stateDefault = 54,
            )
        )

    private fun aliceRoomUser(membership: Membership = JOIN) =
        RoomUser(
            roomId,
            alice,
            "Alice",
            StateEvent(MemberEventContent(membership = membership), EventId(""), alice, roomId, 0L, stateKey = ""),
        )
    ...
    private val roomStore = getInMemoryRoomStore {
        tm.writeTransaction { update(roomId) { Room(roomId, membership = JOIN) } }
    }
    ...
    private val currentSyncState =
        MutableStateFlow(SyncState.RUNNING).also { scheduleSetup { it.value = SyncState.RUNNING } }

    private val userInfo =
        UserInfo(me, "IAmADeviceId", signingPublicKey = Key.Ed25519Key(null, ""), Key.Curve25519Key(null, ""))
    private val cut =
        UserServiceImpl(
            roomStore = roomStore,
    ...
    data class KickLevels(val kickLevel: Long, val myLevel: Long, val banLevel: Long)

    private val myLevelGtKickLevelGtBanLevel = KickLevels(kickLevel = 60L, myLevel = 61L, banLevel = 60L)

    private val myLevelGtKickLevelEqBanLevel = KickLevels(kickLevel = 60L, myLevel = 61L, banLevel = 61L)

    private val myLevelGtKickLevelLtBanLevel = KickLevels(kickLevel = 60L, myLevel = 61L, banLevel = 62L)

    private val myLevelEqKickLevelGtBanLevel = KickLevels(kickLevel = 61L, myLevel = 61L, banLevel = 60L)

    private val myLevelEqKickLevelEqBanLevel = KickLevels(kickLevel = 61L, myLevel = 61L, banLevel = 61L)

    private val myLevelEqKickLevelLtBanLevel = KickLevels(kickLevel = 61L, myLevel = 61L, banLevel = 62L)

    private val myLevelLtKickLevelGtBanLevel = KickLevels(kickLevel = 62L, myLevel = 61L, banLevel = 60L)

    private val myLevelLtKickLevelEqBanLevel = KickLevels(kickLevel = 62L, myLevel = 61L, banLevel = 61L)

    private val myLevelLtKickLevelLtBanLevel = KickLevels(kickLevel = 62L, myLevel = 61L, banLevel = 62L)

    @Test
    fun `canKickUser - return false when not member of room`() = runTest {
        canKickAlice(ownLevel = 56, otherLevel = 54, kickLevel = 55, ownMembership = LEAVE, expect = false)
    }

    @Test
    fun `canKickUser - other user is already LEAVE - return false`() = runTest {
        canKickAlice(ownLevel = 56, otherLevel = 54, kickLevel = 55, otherMembership = LEAVE, expect = false)
    }

    @Test
    fun `canKickUser - my power level lt kick level - return true when my level gt other user level`() = runTest {
        canKickAlice(ownLevel = 56, otherLevel = 54, kickLevel = 55, expect = true)
    }

    @Test
    fun `canKickUser - my power level less than kick level - return false when my level == other user level`() =
        runTest {
            canKickAlice(ownLevel = 56, otherLevel = 56, kickLevel = 55, expect = false)
        }

    @Test
    fun `canKickUser - my power level less than kick level - return false when my level lt other user level`() =
        runTest {
            canKickAlice(ownLevel = 56, otherLevel = 57, kickLevel = 55, expect = false)
        }

    @Test
    fun `canKickUser - my power level == kick level - return true when my level gt other user level`() = runTest {
        canKickAlice(ownLevel = 55, otherLevel = 54, kickLevel = 55, expect = true)
    }

    @Test
    fun `canKickUser - my power level == kick level - return false when my level == other user level`() = runTest {
        canKickAlice(ownLevel = 55, otherLevel = 55, kickLevel = 55, expect = false)
    }

    @Test
    fun `canKickUser - my power level == kick level - return false when my level lt other user level`() = runTest {
        canKickAlice(ownLevel = 55, otherLevel = 56, kickLevel = 55, expect = false)
    }

    @Test
    fun `canKickUser - my power level lt kick level - return false when my level gt other user level`() = runTest {
        canKickAlice(ownLevel = 54, otherLevel = 53, kickLevel = 55, expect = false)
    }

    @Test
    fun `canKickUser - my power level lt kick level - return false when my level == other user level`() = runTest {
        canKickAlice(ownLevel = 54, otherLevel = 54, kickLevel = 55, expect = false)
    }

    @Test
    fun `canKickUser - my power level lt kick level - return false when my level lt other user level`() = runTest {
        canKickAlice(ownLevel = 54, otherLevel = 55, kickLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - not member of room - return false`() = runTest {
        canBanAlice(ownLevel = 56, otherLevel = 54, banLevel = 55, ownMembership = LEAVE, expect = false)
    }

    @Test
    fun `canBanUser - other user already ban - return false`() = runTest {
        canBanAlice(ownLevel = 56, otherLevel = 54, banLevel = 55, otherMembership = BAN, expect = false)
    }

    @Test
    fun `canBanUser - my power level gt ban level - return true when my level gt other user level`() = runTest {
        canBanAlice(ownLevel = 56, otherLevel = 54, banLevel = 55, expect = true)
    }

    @Test
    fun `canBanUser - my power level gt ban level - return false when my level == other user level`() = runTest {
        canBanAlice(ownLevel = 56, otherLevel = 56, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level gt ban level - return false when my level lt other user level`() = runTest {
        canBanAlice(ownLevel = 56, otherLevel = 57, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level == ban level - return true when my level gt other user level`() = runTest {
        canBanAlice(ownLevel = 55, otherLevel = 54, banLevel = 55, expect = true)
    }

    @Test
    fun `canBanUser - my power level == ban level - return false when my level == other user level`() = runTest {
        canBanAlice(ownLevel = 55, otherLevel = 55, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level == ban level - return false when my level lt other user level`() = runTest {
        canBanAlice(ownLevel = 55, otherLevel = 56, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level lt ban level - return false when my level gt other user level`() = runTest {
        canBanAlice(ownLevel = 54, otherLevel = 53, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level lt ban level - return false when my level == other user level`() = runTest {
        canBanAlice(ownLevel = 54, otherLevel = 54, banLevel = 55, expect = false)
    }

    @Test
    fun `canBanUser - my power level lt ban level - return false when my level lt other user level`() = runTest {
        canBanAlice(ownLevel = 54, otherLevel = 55, banLevel = 55, expect = false)
    }

    @Test
    fun `canUnbanUser - not member of room - return false`() = runTest {
        myLevelGtKickLevelGtBanLevel.canUnbanAlice(otherLevel = 60L, ownMembership = LEAVE, expect = false)
    }

    @Test
    fun `canUnbanUser - other user not ban - return false`() = runTest {
        (Membership.entries - BAN).forEach { membership ->
            myLevelGtKickLevelGtBanLevel.canUnbanAlice(otherLevel = 60L, otherMembership = membership, expect = false)
        }
    }
    ...
    fun `canSendEvent - return false when not member of room`() = runTest {
        tm.writeTransaction {
            ...
            val powerLevelsEvent =
                powerLevelsEvent(
                    PowerLevelsEventContent(
                        users = mapOf(me to 55, alice to 50),
    ...
    fun `canSendEvent - be true when allowed to send`() = runTest {
        ...
        tm.writeTransaction { roomStateStore.save(powerLevelsEvent) }
    ...
    fun `canSendEvent - use stateDefault`() = runTest {
        val powerLevelsEvent =
            powerLevelsEvent(PowerLevelsEventContent(users = mapOf(me to 50), events = mapOf(), stateDefault = 55))
        tm.writeTransaction { roomStateStore.save(powerLevelsEvent) }
    ...
    fun `canSendEvent - use eventDefaults`() = runTest {
        val powerLevelsEvent =
            powerLevelsEvent(PowerLevelsEventContent(users = mapOf(me to 50), events = mapOf(), eventsDefault = 55))
        tm.writeTransaction { roomStateStore.save(powerLevelsEvent) }
        cut.canSendEvent<RoomMessageEventContent>(roomId).first() shouldBe false
    }

    @Test
    fun `canSetPowerLevelToMax - other user not member`() = runTest {
        tm.writeTransaction { roomStateStore.save(powerLevelsEvent) }
    ...
    fun `canSetPowerLevelToMax - eventsMap is null - not allow to change the power level when stateDefault value gt own power level`() =
        runTest {
            tm.writeTransaction {
                roomUserStore.update(alice, roomId) { aliceRoomUser() }
                val powerLevelsEvent =
                    powerLevelsEvent(PowerLevelsEventContent(users = mapOf(me to 55, alice to 50), stateDefault = 56))
                roomStateStore.save(powerLevelsEvent)
            }
            cut.canSetPowerLevelToMax(roomId, alice).first() shouldBe null
        }
    ...
    fun `canSetPowerLevelToMax - eventsMap is null - return own power level as max power level value when stateDefault value == own power level`() =
        runTest {
            tm.writeTransaction {
                roomUserStore.update(alice, roomId) { aliceRoomUser() }
                val powerLevelsEvent =
                    powerLevelsEvent(PowerLevelsEventContent(users = mapOf(me to 55, alice to 50), stateDefault = 55))
                roomStateStore.save(powerLevelsEvent)
            }
            cut.canSetPowerLevelToMax(roomId, alice).first() shouldBe PowerLevel.User(55)
        }
        ...
        runTest {
            tm.writeTransaction {
                ...
                val powerLevelsEvent =
                    powerLevelsEvent(
                        PowerLevelsEventContent(
                            users = mapOf(me to 55, alice to 56),
                            events = mapOf(EventType(PowerLevelsEventContent::class, "m.room.power_levels") to 55),
                            stateDefault = 55,
                        )
                    )
        ...
        runTest {
            tm.writeTransaction {
                ...
                val powerLevelsEvent =
                    powerLevelsEvent(
                        PowerLevelsEventContent(
                            users = mapOf(me to 55, alice to 54),
    ...
    fun `canRedactEvent - return false when not member of room`() = runTest {
        tm.writeTransaction {
            ...
            roomStateStore.save(
                StateEvent(
                    content = PowerLevelsEventContent(users = mapOf(me to 40), redact = 30),
                    id = EventId("eventId"),
                    sender = me,
                    roomId = roomId,
                    originTimestamp = 0L,
            ...
            tm.writeTransaction {
                roomStateStore.save(
                    StateEvent(
                        content =
                            PowerLevelsEventContent(
                                users = mapOf(me to 40),
                                events = mapOf(EventType(RedactionEventContent::class, "m.room.redaction") to 30),
                            ),
                        id = EventId("eventId"),
                        sender = me,
                        roomId = roomId,
    ...
    @Test
    fun `canRedactEvent - return false if the user has no high enough power level for event redactions`() = runTest {
        tm.writeTransaction {
            roomStateStore.save(
                StateEvent(
                    content =
                        PowerLevelsEventContent(
                            users = mapOf(me to 20),
                            events = mapOf(EventType(RedactionEventContent::class, "m.room.redaction") to 30),
                        ),
                    id = EventId("eventId"),
                    sender = me,
    ...
    fun `canRedactEvent - return false if the user has no high enough power level for redactions of events of other users`() =
        runTest {
            tm.writeTransaction {
                roomStateStore.save(
                    StateEvent(
                        content = PowerLevelsEventContent(users = mapOf(me to 20), redact = 30),
                        id = EventId("eventId"),
                        sender = me,
                        roomId = roomId,
                        originTimestamp = 0L,
    ...
    @Test
    fun `getUserPresence - last active is below threshold - should pass from store and make null later`() = runTest {
        val presence =
            UserPresence(
                presence = Presence.ONLINE,
                lastUpdate = clock.now(),
        ...
        tm.writeTransaction { userPresenceStore.setPresence(alice, presence2) }
        delay(100.milliseconds)
        result.value shouldBe presence2
    }

    @Test
    fun `getUserPresence - last active is below threshold - should fallback to last update`() = runTest {
        val presence =
            UserPresence(presence = Presence.ONLINE, lastUpdate = clock.now() - 4.minutes, statusMessage = "status")
        tm.writeTransaction { userPresenceStore.setPresence(alice, presence) }
        val result = cut.getPresence(alice).stateIn(backgroundScope)
        result.value shouldBe presence
    ...
    @Test
    fun `getUserPresence - last active is above threshold - should be null`() = runTest {
        val presence =
            UserPresence(presence = Presence.ONLINE, lastUpdate = clock.now() - 6.minutes, statusMessage = "status")
        tm.writeTransaction { userPresenceStore.setPresence(alice, presence) }
        cut.getPresence(alice).first() shouldBe null
    }
    ...
    private suspend fun canKickAlice(
        ...
        val powerLevelsEvent =
            powerLevelsEvent(
                PowerLevelsEventContent(users = mapOf(me to ownLevel, alice to otherLevel), kick = kickLevel)
            )
        tm.writeTransaction {
            roomStateStore.save(powerLevelsEvent)
    ...
    private suspend fun canBanAlice(
        ...
        val powerLevelsEvent =
            powerLevelsEvent(
                PowerLevelsEventContent(users = mapOf(me to ownLevel, alice to otherLevel), ban = banLevel)
            )
        tm.writeTransaction {
            roomStateStore.save(powerLevelsEvent)
            roomStore.update(roomId) { Room(roomId, membership = ownMembership) }
    ...
    private suspend fun canInvite(
        ...
        val powerLevelsEvent =
            powerLevelsEvent(PowerLevelsEventContent(users = mapOf(me to ownLevel), invite = inviteLevel))
        tm.writeTransaction {
            roomStateStore.save(powerLevelsEvent)
            roomStore.update(roomId) { Room(roomId, membership = ownMembership) }
        }
    ...
    private suspend fun KickLevels.canUnbanAlice(
        ...
        tm.writeTransaction {
            ...
            val powerLevelsEvent =
                powerLevelsEvent(
                    PowerLevelsEventContent(
                        users = mapOf(me to myLevel, alice to otherLevel),
                        kick = kickLevel,
                        ban = banLevel,
                    )
                )
    ...
    private fun powerLevelsEvent(powerLevelsEventContent: PowerLevelsEventContent) =
        StateEvent(powerLevelsEventContent, EventId("\$event"), me, roomId, 1234, stateKey = "")
}
