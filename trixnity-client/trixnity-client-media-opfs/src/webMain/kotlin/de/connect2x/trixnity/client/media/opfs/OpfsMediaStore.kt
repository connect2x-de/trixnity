...
internal class OpfsMediaStore(
    ...
    override suspend fun init(coroutineScope: CoroutineScope) {
        ...
        coroutineScope.coroutineContext.job.invokeOnCompletion {
            @OptIn(DelicateCoroutinesApi::class) GlobalScope.launch { delTmp() }
        }
    ...
    override suspend fun getMedia(url: String): OpfsPlatformMedia? =
        basePathLock.withLock(url) {
            val fileHandle =
                try {
    ...
    private inner class FileBasedOpfsPlatformMediaImpl(private val url: String, private val file: File) :
        OpfsPlatformMedia {
        ...
        override suspend fun getTemporaryFile(): Result<OpfsPlatformMedia.TemporaryFile> = runCatching {
            basePathLock.withLock(url) {
                val tmpPath = tmpPath()
    ...
    private inner class OpfsPlatformMediaImpl(private val url: String, private val delegate: ByteArrayFlow) :
    ...
    private inner class OpfsPlatformMediaTemporaryFileImpl(override val file: File, private val fileName: String) :
...
fun MediaStoreModule.Companion.opfs(basePath: FileSystemDirectoryHandle) = MediaStoreModule {
    module {
        single<MediaStore> {
            OpfsMediaStore(basePath = basePath, coroutineScope = get(), configuration = get(), clock = get())
        }
    }
}

internal fun FileSystemRemoveOptions(recursive: Boolean? = null): FileSystemRemoveOptions = unsafeJso {
    recursive?.also { this.recursive = it }
}

internal fun FileSystemGetFileOptions(create: Boolean? = null): FileSystemGetFileOptions = unsafeJso {
    create?.also { this.create = it }
}

internal fun FileSystemGetDirectoryOptions(create: Boolean? = null): FileSystemGetDirectoryOptions = unsafeJso {
    create?.also { this.create = it }
}
