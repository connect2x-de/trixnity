...
class ExposedStoreWriteTransaction(cacheTransaction: CacheTransaction) :
...
class ExposedStoreTransactionManager(private val database: R2dbcDatabase) : StoreTransactionManager() {
    override suspend fun <T> repositoryReadTransaction(block: suspend StoreReadTransaction.() -> T): T =
        suspendTransaction(database, readOnly = true) { block(NoOpStoreReadTransaction) }

    override suspend fun <T> repositoryWriteTransaction(
        cacheTransaction: CacheTransaction,
        block: suspend StoreWriteTransaction.() -> T,
    ): T = suspendTransaction(database, readOnly = false) { block(ExposedStoreWriteTransaction(cacheTransaction)) }
}
