...
data class RoomCrossSigningKeys(@PrimaryKey val userId: UserId, val value: String)
...
interface CrossSigningKeysDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomCrossSigningKeys)

    @Query("DELETE FROM CrossSigningKeys WHERE userId = :userId") suspend fun delete(userId: UserId)

    @Query("DELETE FROM CrossSigningKeys") suspend fun deleteAll()
}

internal class RoomCrossSigningKeysRepository(db: TrixnityRoomDatabase, private val json: Json) :
    ...
    override suspend fun get(key: UserId): Set<StoredCrossSigningKeys>? =
        dao.get(key)?.let { entity -> json.decodeFromString<Set<StoredCrossSigningKeys>>(entity.value) }

    context(transaction: WriteTransaction)
    override suspend fun save(key: UserId, value: Set<StoredCrossSigningKeys>) =
        dao.insert(RoomCrossSigningKeys(userId = key, value = json.encodeToString(value)))
