...
@Entity(tableName = "KeyVerificationState", primaryKeys = ["keyId", "keyAlgorithm"])
...
interface KeyVerificationStateDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomKeyVerificationState)
    ...
    @Query("DELETE FROM KeyVerificationState") suspend fun deleteAll()
}

internal class RoomKeyVerificationStateRepository(db: TrixnityRoomDatabase, private val json: Json) :
    ...
    override suspend fun get(key: KeyVerificationStateKey): KeyVerificationState? =
        dao.get(key.keyId, key.keyAlgorithm)?.let { json.decodeFromString(it.verificationState) }
    ...
    context(transaction: WriteTransaction)
    override suspend fun delete(key: KeyVerificationStateKey) = dao.delete(key.keyId, key.keyAlgorithm)
