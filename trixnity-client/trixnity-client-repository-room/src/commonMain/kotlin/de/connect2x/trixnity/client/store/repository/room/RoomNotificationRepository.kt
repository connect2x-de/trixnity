...
interface NotificationDao {
    @Query("SELECT * FROM Notification") suspend fun getAll(): List<RoomNotification>

    @Query("SELECT * FROM Notification WHERE id = :id LIMIT 1") suspend fun get(id: String): RoomNotification?

    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomNotification)

    @Query("DELETE FROM Notification WHERE id = :id") suspend fun delete(id: String)

    @Query("DELETE FROM Notification WHERE roomId = :roomId ") suspend fun delete(roomId: RoomId)

    @Query("DELETE FROM Notification") suspend fun deleteAll()
}

internal class RoomNotificationRepository(db: TrixnityRoomDatabase, private val json: Json) : NotificationRepository {
    ...
    override suspend fun save(key: String, value: StoredNotification) =
        dao.insert(RoomNotification(id = key, roomId = value.roomId, value = json.encodeToString(value)))
