...
@Entity(tableName = "NotificationState", primaryKeys = ["roomId"])
data class RoomNotificationState(val roomId: RoomId, val value: String)

@Dao
interface NotificationStateDao {
    @Query("SELECT * FROM NotificationState") suspend fun getAll(): List<RoomNotificationState>

    @Query("SELECT * FROM NotificationState WHERE roomId = :roomId LIMIT 1")
    suspend fun get(roomId: RoomId): RoomNotificationState?

    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomNotificationState)

    @Query("DELETE FROM NotificationState WHERE roomId = :roomId") suspend fun delete(roomId: RoomId)

    @Query("DELETE FROM NotificationState") suspend fun deleteAll()
}

internal class RoomNotificationStateRepository(db: TrixnityRoomDatabase, private val json: Json) :
    ...
    override suspend fun save(key: RoomId, value: StoredNotificationState) =
        dao.insert(RoomNotificationState(roomId = value.roomId, value = json.encodeToString(value)))
