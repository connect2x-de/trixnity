...
@Entity(tableName = "NotificationUpdate", primaryKeys = ["id"])
data class RoomNotificationUpdate(val id: String, val roomId: RoomId, val value: String)

@Dao
interface NotificationUpdateDao {
    @Query("SELECT * FROM NotificationUpdate") suspend fun getAll(): List<RoomNotificationUpdate>

    @Query("SELECT * FROM NotificationUpdate WHERE id = :id LIMIT 1")
    suspend fun get(id: String): RoomNotificationUpdate?

    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomNotificationUpdate)

    @Query("DELETE FROM NotificationUpdate WHERE id = :id") suspend fun delete(id: String)

    @Query("DELETE FROM NotificationUpdate WHERE roomId = :roomId ") suspend fun delete(roomId: RoomId)

    @Query("DELETE FROM NotificationUpdate") suspend fun deleteAll()
}

internal class RoomNotificationUpdateRepository(db: TrixnityRoomDatabase, private val json: Json) :
    ...
    override suspend fun save(key: String, value: StoredNotificationUpdate) =
        dao.insert(RoomNotificationUpdate(id = key, roomId = value.roomId, value = json.encodeToString(value)))
