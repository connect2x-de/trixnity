...
data class RoomOutboundMegolmSession(@PrimaryKey val roomId: RoomId, val value: String)
...
interface OutboundMegolmSessionDao {
    ...
    @Query("SELECT * FROM OutboundMegolmSession") suspend fun getAll(): List<RoomOutboundMegolmSession>

    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomOutboundMegolmSession)

    @Query("DELETE FROM OutboundMegolmSession WHERE roomId = :roomId") suspend fun delete(roomId: RoomId)

    @Query("DELETE FROM OutboundMegolmSession") suspend fun deleteAll()
}

internal class RoomOutboundMegolmSessionRepository(db: TrixnityRoomDatabase, private val json: Json) :
    ...
    override suspend fun save(key: RoomId, value: StoredOutboundMegolmSession) =
        dao.insert(RoomOutboundMegolmSession(roomId = key, value = json.encodeToString(value)))
