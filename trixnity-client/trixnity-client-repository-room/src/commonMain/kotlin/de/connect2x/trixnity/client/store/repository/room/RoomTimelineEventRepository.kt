...
@Entity(tableName = "TimelineEvent", primaryKeys = ["roomId", "eventId"])
...
interface TimelineEventDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(entity: RoomTimelineEvent)

    @Query("DELETE FROM TimelineEvent WHERE roomId = :roomId") suspend fun delete(roomId: RoomId)

    @Query("DELETE FROM TimelineEvent WHERE roomId = :roomId AND eventId = :eventId")
    suspend fun delete(roomId: RoomId, eventId: EventId)

    @Query("DELETE FROM TimelineEvent") suspend fun deleteAll()
}

internal class RoomTimelineEventRepository(db: TrixnityRoomDatabase, private val json: Json) : TimelineEventRepository {
    ...
    override suspend fun get(key: TimelineEventKey): TimelineEvent? =
        dao.get(key.roomId, key.eventId)?.let { entity -> json.decodeFromString(entity.value) }

    context(transaction: WriteTransaction)
    override suspend fun save(key: TimelineEventKey, value: TimelineEvent) =
        dao.insert(RoomTimelineEvent(roomId = key.roomId, eventId = key.eventId, value = json.encodeToString(value)))

    context(transaction: WriteTransaction)
    override suspend fun delete(key: TimelineEventKey) = dao.delete(key.roomId, key.eventId)
