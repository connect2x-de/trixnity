...
interface UserApiClient {
    ...
    /** @see [GetProfileField] */
    suspend fun <T : ProfileField> getProfileField(userId: UserId, key: ProfileField.Key<T>): Result<T>

    /** @see [SetProfileField] */
    suspend fun setProfileField(userId: UserId, field: ProfileField): Result<Unit>

    /** @see [DeleteProfileField] */
    suspend fun deleteProfileField(userId: UserId, key: ProfileField.Key<*>): Result<Unit>

    /** @see [GetProfile] */
    suspend fun getProfile(userId: UserId): Result<Profile>

    /** @see [GetPresence] */
    suspend fun getPresence(userId: UserId): Result<PresenceEventContent>

    /** @see [SetPresence] */
    suspend fun setPresence(userId: UserId, presence: Presence, statusMessage: String? = null): Result<Unit>
    ...
    /**
     * This splits [events] into multiple requests, when they have a different type (for example a mix of encrypted and
     * unencrypted events).
     *
     * @see [SendToDevice]
     */
    ...
    /** @see [GetFilter] */
    suspend fun getFilter(userId: UserId, filterId: String): Result<Filters>

    /** @see [SetFilter] */
    suspend fun setFilter(userId: UserId, filters: Filters): Result<String>

    /** @see [GetGlobalAccountData] */
    suspend fun getAccountData(type: String, userId: UserId, key: String = ""): Result<GlobalAccountDataEventContent>
...
class UserApiClientImpl(
    ...
    override suspend fun <T : ProfileField> getProfileField(userId: UserId, key: ProfileField.Key<T>): Result<T> =
        @Suppress("UNCHECKED_CAST")
        baseClient.request(GetProfileField(userId, key)).map {
            if (it.key != key) error("unexpected type ${it.key}, expected $key")
            else it as? T ?: error("unexpected type ${it::class}, expected ${key::class}")
        }

    override suspend fun setProfileField(userId: UserId, field: ProfileField): Result<Unit> =
        baseClient.request(SetProfileField(userId, field.key), field)

    override suspend fun deleteProfileField(userId: UserId, key: ProfileField.Key<*>): Result<Unit> =
        baseClient.request(DeleteProfileField(userId, key))

    override suspend fun getProfile(userId: UserId): Result<Profile> = baseClient.request(GetProfile(userId))

    override suspend fun getPresence(userId: UserId): Result<PresenceEventContent> =
        baseClient.request(GetPresence(userId))

    override suspend fun setPresence(userId: UserId, presence: Presence, statusMessage: String?): Result<Unit> =
    ...
    override suspend fun <C : ToDeviceEventContent> sendToDeviceUnsafe(
        ...
        require(events.flatMap { it.value.values }.all { it.instanceOf(firstEventForType::class) }) {
    ...
    override suspend fun sendToDeviceUnsafe(
        ...
        events: Map<UserId, Map<String, ToDeviceEventContent>>,
        transactionId: String,
    ): Result<Unit> = baseClient.request(SendToDevice(type, transactionId), SendToDevice.Request(events))

    override suspend fun sendToDevice(
        events: Map<UserId, Map<String, ToDeviceEventContent>>,
        transactionId: String,
    ): Result<Unit> = runCatching {
        data class FlatEntry(val userId: UserId, val deviceId: String, val event: ToDeviceEventContent)

        val flatEvents = events.flatMap { (userId, deviceEvents) ->
            deviceEvents.map { (deviceId, deviceEvent) -> FlatEntry(userId, deviceId, deviceEvent) }
        }
        if (flatEvents.isNotEmpty()) {
            ...
            coroutineScope {
                eventsByType.values.forEach { launch { sendToDeviceUnsafe(it, transactionId).getOrThrow() } }
            }
        }
    }

    override suspend fun getFilter(userId: UserId, filterId: String): Result<Filters> =
        baseClient.request(GetFilter(userId, filterId))

    override suspend fun setFilter(userId: UserId, filters: Filters): Result<String> =
    ...
    override suspend fun setAccountData(
        ...
        val eventType =
            contentMappings.globalAccountData.contentType(content).let { type ->
                if (key.isEmpty()) type else type.removeSuffix("*") + key
            }
