...
/**
 * Represents the flow for handling OAuth 2.0 authorization code based login operations. This interface provides methods
 * to facilitate the redirection and callback required during the OAuth 2.0 authorization process.
 *
...
interface OAuth2AuthorizationCodeLoginFlow {
    /**
     * Initiates the authentication process. This method generates an authentication request containing a URL and
     * associated metadata necessary when the app has been killed during the authentication process.
     */
    suspend fun createAuthRequest(): Result<AuthRequestData>

    /**
     * Represents the data required for initiating an authentication process. It includes the URL for the authentication
     * request and serializable state information to continue authentication after the app has been killed.
     */
    data class AuthRequestData(val url: String, val state: State) {
        /** Represents the state information used during an authentication flow. */
        @Serializable data class State(val clientId: String, val state: String, val codeVerifier: String)
    }
...
class OAuth2AuthorizationCodeLoginFlowImpl(
    ...
    private val state =
        initialState?.state ?: Uuid.fromByteArray(SecureRandom.nextBytes(Uuid.SIZE_BYTES)).toHexDashString()
    private val codeVerifier = initialState?.codeVerifier ?: SecureRandom.nextString(64)
        ...
        OAuth2ApiClient(
            ...
            .use {
                ...
                val authenticationRequestUrl =
                    URLBuilder(getServerMetadata().authorizationEndpoint)
                        .apply {
