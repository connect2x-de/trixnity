...
class ServerApiClientTest : TrixnityBaseTest() {
    ...
    fun shouldGetVersions() = runTest {
        val response = GetVersions.Response(versions = emptyList(), unstableFeatures = mapOf())
        ...
        val matrixRestClient =
            MatrixClientServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/client/v3/search", request.url.fullPath)
                            ...
                            Search.Request.Categories.RoomEventsCriteria(
                                groupings =
                                    Search.Request.Categories.RoomEventsCriteria.Groupings(
                                        setOf(Search.Request.Categories.RoomEventsCriteria.Groupings.Groups("room_id"))
                                    ),
