...
class AppserviceRoutesTest : TrixnityBaseTest() {
    ...
    private fun ApplicationTestBuilder.initCut() {
        application {
            ...
            matrixApiServer(json) { appserviceApiRoutes(handlerMock, json, mapping) }
        }
    }
    ...
    fun shouldPing() = testApplication {
        initCut()
        everySuspend { handlerMock.ping(any()) }.returns(Ping.Response(1234))
        val response =
            client.post("/_matrix/client/v1/appservice/appId/ping") {
                bearerAuth("token")
