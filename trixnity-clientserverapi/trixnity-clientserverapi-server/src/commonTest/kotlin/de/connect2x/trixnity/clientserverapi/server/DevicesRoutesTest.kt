...
class DevicesRoutesTest : TrixnityBaseTest() {
    ...
    private fun ApplicationTestBuilder.initCut() {
        application {
            ...
            matrixApiServer(json) { deviceApiRoutes(handlerMock, json, mapping) }
        }
    }
    ...
    fun shouldGetDevices() = testApplication {
        ...
        verifySuspend { handlerMock.getDevices(any()) }
    }
    ...
    fun shouldGetDevice() = testApplication {
        ...
        verifySuspend { handlerMock.getDevice(assert { it.endpoint.deviceId shouldBe "ABCDEF" }) }
    }

    @Test
    fun shouldUpdateDevice() = testApplication {
        initCut()
        everySuspend { handlerMock.updateDevice(any()) }.returns(Unit)
        val response =
            client.put("/_matrix/client/v3/devices/ABCDEF") {
                bearerAuth("token")
    ...
    fun shouldDeleteDevices() = testApplication {
        initCut()
        everySuspend { handlerMock.deleteDevices(any()) }.returns(ResponseWithUIA.Success(Unit))
        val response =
            client.post("/_matrix/client/v3/delete_devices") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
                setBody(
                    """
        ...
        verifySuspend {
            handlerMock.deleteDevices(
                assert { it.requestBody shouldBe RequestWithUIA(DeleteDevices.Request(listOf("ABCDEFG")), null) }
            )
        }
    }
    ...
    fun shouldDeleteDevice() = testApplication {
        initCut()
        everySuspend { handlerMock.deleteDevice(any()) }.returns(ResponseWithUIA.Success(Unit))
        val response =
            client.delete("/_matrix/client/v3/devices/ABCDEF") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
                setBody("{}")
            }
        ...
        verifySuspend { handlerMock.deleteDevice(assert { it.endpoint.deviceId shouldBe "ABCDEF" }) }
    }
    ...
    fun shouldGetDehydratedDevice() = testApplication {
        ...
        verifySuspend { handlerMock.getDehydratedDevice(any()) }
    }
    ...
    fun shouldSetDehydratedDevice() = testApplication {
        initCut()
        everySuspend { handlerMock.setDehydratedDevice(any()) }.returns(SetDehydratedDevice.Response("ABCDEFG"))
        val response =
        ...
        verifySuspend {
            handlerMock.setDehydratedDevice(
                assert {
                    it.requestBody shouldBe
                        SetDehydratedDevice.Request(
                            deviceId = "ABCDEFG",
                            deviceData =
                                DehydratedDeviceData.DehydrationV2("encrypted dehydrated device", "random nonce"),
                            deviceKeys =
                                Signed(
                                    DeviceKeys(
    ...
    fun shouldDeleteDehydratedDevice() = testApplication {
        initCut()
        everySuspend { handlerMock.deleteDehydratedDevice(any()) }.returns(DeleteDehydratedDevice.Response("ABCDEFG"))
        val response =
        ...
        verifySuspend { handlerMock.deleteDehydratedDevice(any()) }
    }
    ...
    fun shouldGetDehydratedDeviceEvents() = testApplication {
        ...
        val response =
            client.post("/_matrix/client/unstable/org.matrix.msc3814.v1/dehydrated_device/ABCDEFG/events") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
        ...
        verifySuspend {
            handlerMock.getDehydratedDeviceEvents(
                assert {
