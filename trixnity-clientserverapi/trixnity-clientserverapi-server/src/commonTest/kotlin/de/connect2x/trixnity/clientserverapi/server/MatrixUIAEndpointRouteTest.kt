...
class MatrixUIAEndpointRouteTest : TrixnityBaseTest() {
        ...
        application {
            matrixApiServer(json) {
                matrixUIAEndpoint<PostPath, PostPath.Request, PostPath.Response>(json, mapping) {
                    ...
                    requestBody.authentication shouldBe
                        AuthenticationRequestWithSession(
                            AuthenticationRequest.Password(IdentifierType.User("user"), "password"),
                            "session",
                        )
                    ResponseWithUIA.Success(PostPath.Response("dino"))
                }
            }
        }
        val response =
                            ...
                            parameter =
                                mapOf(
                                    AuthenticationType.Unknown("example.type.baz") to
                                        UIAState.Parameter.Unknown(
                                            buildJsonObject { put("example_key", JsonPrimitive("foobar")) }
                                        ),
    ...
    fun shouldHandleRequestWithErrorResponse() = testApplication {
        ...
        response.body<String>() shouldBe
            """{"completed":[],"flows":[{"stages":["m.login.password"]}],"session":"session","errcode":"M_UNAUTHORIZED","error":"password wrong"}"""
        response.contentType() shouldBe ContentType.Application.Json
        response.status shouldBe HttpStatusCode.Unauthorized
    }
        ...
        application {
            matrixApiServer(json) {
                matrixUIAEndpoint<GetPath, Unit, GetPath.Response>(json, mapping) {
                    ...
                    ResponseWithUIA.Success(GetPath.Response("anti-dino"))
                }
                matrixUIAEndpoint<PostPath, PostPath.Request, PostPath.Response>(json, mapping) {
                    ResponseWithUIA.Success(PostPath.Response("dino"))
                }
