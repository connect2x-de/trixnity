...
import de.connect2x.trixnity.core.model.push.*
...
class PushRoutesTest : TrixnityBaseTest() {
    ...
    private fun ApplicationTestBuilder.initCut() {
        application {
            ...
            matrixApiServer(json) { pushApiRoutes(handlerMock, json, mapping) }
        }
    }
    ...
    fun shouldGetPushers() = testApplication {
                ...
                GetPushers.Response(
                    listOf(
                        GetPushers.Response.Pusher(
                            ...
                            data = PusherData(format = "format", url = "https://example.com/_matrix/push/v1/notify"),
        ...
        verifySuspend { handlerMock.getPushers(any()) }
    }

    @Test
    fun shouldSetPushersSet() = testApplication {
        initCut()
        everySuspend { handlerMock.setPushers(any()) }.returns(Unit)
        val response =
            client.post("/_matrix/client/v3/pushers/set") {
                bearerAuth("token")
    ...
    fun shouldGetPushRules() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/pushrules/") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend { handlerMock.getPushRules(any()) }
    }
    ...
    fun shouldGetPushRule() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/pushrules/scope/content/ruleId") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
    ...
    fun shouldSetPushRule() = testApplication {
        initCut()
        everySuspend { handlerMock.setPushRule(any()) }.returns(Unit)
        val response =
            client.put("/_matrix/client/v3/pushrules/scope/content/ruleId?before=before&after=after") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
                setBody(
                    """
    ...
    fun shouldDeletePushRule() = testApplication {
        initCut()
        everySuspend { handlerMock.deletePushRule(any()) }.returns(Unit)
        val response = client.delete("/_matrix/client/v3/pushrules/scope/content/ruleId") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe "{}"
    ...
    fun shouldGetPushRuleActions() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/pushrules/scope/content/ruleId/actions") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend {
            handlerMock.getPushRuleActions(
                assert {
                    it.endpoint.scope shouldBe "scope"
                    it.endpoint.kind shouldBe PushRuleKind.CONTENT
                    it.endpoint.ruleId shouldBe "ruleId"
    ...
    fun shouldSetPushRuleActions() = testApplication {
        initCut()
        everySuspend { handlerMock.setPushRuleActions(any()) }.returns(Unit)
        val response =
            client.put("/_matrix/client/v3/pushrules/scope/content/ruleId/actions") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
                setBody(
                    """
        ...
        verifySuspend {
            handlerMock.setPushRuleActions(
                assert {
                    it.endpoint.scope shouldBe "scope"
                    it.endpoint.kind shouldBe PushRuleKind.CONTENT
                    it.endpoint.ruleId shouldBe "ruleId"
                    it.requestBody shouldBe
                        SetPushRuleActions.Request(setOf(PushAction.Notify, PushAction.SetSoundTweak("default")))
                }
            )
        }
    }
    ...
    fun shouldGetPushRuleEnabled() = testApplication {
        initCut()
        everySuspend { handlerMock.getPushRuleEnabled(any()) }.returns(GetPushRuleEnabled.Response(false))
        val response = client.get("/_matrix/client/v3/pushrules/scope/content/ruleId/enabled") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend {
            handlerMock.getPushRuleEnabled(
                assert {
                    it.endpoint.scope shouldBe "scope"
                    it.endpoint.kind shouldBe PushRuleKind.CONTENT
                    it.endpoint.ruleId shouldBe "ruleId"
    ...
    fun shouldSetPushRuleEnabled() = testApplication {
        initCut()
        everySuspend { handlerMock.setPushRuleEnabled(any()) }.returns(Unit)
        val response =
            client.put("/_matrix/client/v3/pushrules/scope/content/ruleId/enabled") {
                bearerAuth("token")
                contentType(ContentType.Application.Json)
                setBody(
                    """
        ...
        verifySuspend {
            handlerMock.setPushRuleEnabled(
                assert {
                    it.endpoint.scope shouldBe "scope"
                    it.endpoint.kind shouldBe PushRuleKind.CONTENT
                    it.endpoint.ruleId shouldBe "ruleId"
