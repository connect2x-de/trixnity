...
class RoomsRoutesTest : TrixnityBaseTest() {
    ...
    private fun ApplicationTestBuilder.initCut() {
        application {
            ...
            matrixApiServer(json) { roomApiRoutes(handlerMock, json, mapping) }
        }
    }
    ...
    fun shouldGetStateEventContent() = testApplication {
        ...
        val response =
            client.get("/_matrix/client/v3/rooms/!room:server/state/m.room.name/?format=content") {
                bearerAuth("token")
            }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
    ...
    fun shouldGetState() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/rooms/!room:server/state") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend { handlerMock.getState(assert { it.endpoint.roomId shouldBe RoomId("!room:server") }) }
    }
    ...
    fun shouldGetMembers() = testApplication {
        ...
        val response =
            client.get("/_matrix/client/v3/rooms/!room:server/members?at=someAt&membership=join") {
                bearerAuth("token")
            }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
    ...
    fun shouldGetJoinedMembers() = testApplication {
        ...
        everySuspend { handlerMock.getJoinedMembers(any()) }
            .returns(
                GetJoinedMembers.Response(
                    joined =
                        mapOf(
                            UserId("user1", "server") to GetJoinedMembers.Response.RoomMember("Unicorn"),
                            UserId("user2", "server") to GetJoinedMembers.Response.RoomMember("Dino"),
                        )
                )
            )
        val response = client.get("/_matrix/client/v3/rooms/!room:server/joined_members") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend { handlerMock.getJoinedMembers(assert { it.endpoint.roomId shouldBe RoomId("!room:server") }) }
    }
    ...
    fun shouldGetEvents() = testApplication {
        ...
        val response =
            client.get("/_matrix/client/v3/rooms/!room:server/messages?from=from&dir=f&limit=10") {
                bearerAuth("token")
            }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
    ...
    fun shouldGetRelations() = testApplication {
        ...
        verifySuspend {
            handlerMock.getRelations(
                assert {
                    it.endpoint shouldBe
                        GetRelations(
                            roomId = RoomId("!room:server"),
    ...
    fun shouldGetRelationsByRelationType() = testApplication {
        ...
        verifySuspend {
            handlerMock.getRelationsByRelationType(
                assert {
                    it.endpoint shouldBe
    ...
    fun shouldGetRelationsByRelationTypeAndEventType() = testApplication {
        ...
        verifySuspend {
            handlerMock.getRelationsByRelationTypeAndEventType(
                assert {
                    it.endpoint shouldBe
    ...
    fun shouldSendMessageEvent() = testApplication {
        initCut()
        everySuspend { handlerMock.sendMessageEvent(any()) }.returns(SendEventResponse(EventId("event")))
        val response =
            client.put(
    ...
    fun shouldSendRedactEvent() = testApplication {
        initCut()
        everySuspend { handlerMock.redactEvent(any()) }.returns(SendEventResponse(EventId("event")))
        val response =
    ...
    fun shouldCreateRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.createRoom(any()) }.returns(CreateRoom.Response(RoomId("!room:server")))
        val response =
    ...
    fun shouldSetRoomAlias() = testApplication {
        initCut()
        everySuspend { handlerMock.setRoomAlias(any()) }.returns(Unit)
        val response =
    ...
    fun shouldGetRoomAlias() = testApplication {
        ...
        everySuspend { handlerMock.getRoomAlias(any()) }
            .returns(GetRoomAlias.Response(roomId = RoomId("!room:server"), servers = listOf("server1", "server2")))
        val response = client.get("/_matrix/client/v3/directory/room/%23unicorns:server") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend {
            handlerMock.getRoomAlias(assert { it.endpoint.roomAliasId shouldBe RoomAliasId("unicorns", "server") })
        }
    }
    ...
    fun shouldGetRoomAliases() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/rooms/!room:server/aliases") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend { handlerMock.getRoomAliases(assert { it.endpoint.roomId shouldBe RoomId("!room:server") }) }
    }

    @Test
    fun shouldDeleteRoomAlias() = testApplication {
        initCut()
        everySuspend { handlerMock.deleteRoomAlias(any()) }.returns(Unit)
        val response = client.delete("/_matrix/client/v3/directory/room/%23unicorns:server") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe "{}"
        ...
        verifySuspend {
            handlerMock.deleteRoomAlias(assert { it.endpoint.roomAliasId shouldBe RoomAliasId("unicorns", "server") })
        }
    }
    ...
    fun shouldGetJoinedRooms() = testApplication {
        ...
        everySuspend { handlerMock.getJoinedRooms(any()) }
            .returns(GetJoinedRooms.Response(setOf(RoomId("!room1:server"), RoomId("!room2:server"))))
        val response = client.get("/_matrix/client/v3/joined_rooms") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend { handlerMock.getJoinedRooms(any()) }
    }

    @Test
    fun shouldInviteUser() = testApplication {
        initCut()
        everySuspend { handlerMock.inviteUser(any()) }.returns(Unit)
        val response =
    ...
    fun shouldKickUser() = testApplication {
        initCut()
        everySuspend { handlerMock.kickUser(any()) }.returns(Unit)
        val response =
    ...
    fun shouldBanUser() = testApplication {
        initCut()
        everySuspend { handlerMock.banUser(any()) }.returns(Unit)
        val response =
    ...
    fun shouldUnbanUser() = testApplication {
        initCut()
        everySuspend { handlerMock.unbanUser(any()) }.returns(Unit)
        val response =
    ...
    fun shouldJoinRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.joinRoom(any()) }.returns(JoinRoom.Response(RoomId("!room:server")))
        val response =
                        ...
                        JoinRoom.Request(
                            thirdPartySigned =
                                Signed(
                                    ...
                                    mapOf("example.org" to keysOf(Key.Ed25519Key("0", "some9signature"))),
                                ),
                            reason = null,
                        )
                }
    ...
    fun shouldJoinRoomVia() = testApplication {
        initCut()
        everySuspend { handlerMock.joinRoomVia(any()) }.returns(JoinRoomVia.Response(RoomId("!room:server")))
        val response =
    ...
    fun shouldKnockRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.knockRoom(any()) }.returns(KnockRoom.Response(RoomId("!room:server")))
        val response =
    ...
    fun shouldLeaveRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.leaveRoom(any()) }.returns(Unit)
        val response =
    ...
    fun shouldForgetRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.forgetRoom(any()) }.returns(Unit)
        val response = client.post("/_matrix/client/v3/rooms/!room:server/forget") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe "{}"
        }
        verifySuspend { handlerMock.forgetRoom(assert { it.endpoint.roomId shouldBe RoomId("!room:server") }) }
    }

    @Test
    fun shouldSetReceipt() = testApplication {
        initCut()
        everySuspend { handlerMock.setReceipt(any()) }.returns(Unit)
        val response =
    ...
    fun shouldSetReadMarkers() = testApplication {
        initCut()
        everySuspend { handlerMock.setReadMarkers(any()) }.returns(Unit)
        val response =
        ...
        verifySuspend {
            handlerMock.setReadMarkers(
                assert {
                    it.endpoint.roomId shouldBe RoomId("!room:server")
                    it.requestBody shouldBe
                        SetReadMarkers.Request(fullyRead = EventId("$1event"), read = EventId("$2event"))
                }
            )
        }
    }
    ...
    fun shouldGetAccountData() = testApplication {
        initCut()
        everySuspend { handlerMock.getAccountData(any()) }.returns(FullyReadEventContent(EventId("$1event")))
        val response =
    ...
    fun shouldGetAccountDataWithKey() = testApplication {
        ...
        val response =
            client.get(
                "/_matrix/client/v3/user/@alice:example.com/rooms/!room:server/account_data/m.fully_read-readkey"
            ) {
                bearerAuth("token")
            }
        assertSoftly(response) {
    ...
    fun shouldSetTyping() = testApplication {
        initCut()
        everySuspend { handlerMock.setTyping(any()) }.returns(Unit)
        val response =
    ...
    fun shouldGetDirectoryVisibility() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/directory/list/room/!room:server")
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
        ...
        verifySuspend {
            handlerMock.getDirectoryVisibility(assert { it.endpoint.roomId shouldBe RoomId("!room:server") })
        }
    }
    ...
    fun shouldSetDirectoryVisibility() = testApplication {
        initCut()
        everySuspend { handlerMock.setDirectoryVisibility(any()) }.returns(Unit)
        val response =
        ...
        verifySuspend {
            handlerMock.setDirectoryVisibility(
                assert {
                    it.endpoint.roomId shouldBe RoomId("!room:server")
    ...
    fun shouldGetPublicRooms() = testApplication {
        ...
        val response = client.get("/_matrix/client/v3/publicRooms?limit=5&server=example&since=since")
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
    ...
    fun shouldSetTag() = testApplication {
        initCut()
        everySuspend { handlerMock.setTag(any()) }.returns(Unit)
        val response =
    ...
    fun shouldDeleteTag() = testApplication {
        initCut()
        everySuspend { handlerMock.deleteTag(any()) }.returns(Unit)
        val response =
            client.delete("/_matrix/client/v3/user/@user:server/rooms/!room:server/tags/m.dino") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe "{}"
    ...
    fun shouldGetEventContext() = testApplication {
        ...
        val response =
            client.get("/_matrix/client/v3/rooms/!room:server/context/event?filter=filter&limit=10") {
                bearerAuth("token")
            }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
    ...
    fun shouldReportRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.reportRoom(any()) }.returns(Unit)
        val response =
    ...
    fun shouldReportEvent() = testApplication {
        initCut()
        everySuspend { handlerMock.reportEvent(any()) }.returns(Unit)
        val response =
    ...
    fun shouldUpgradeRoom() = testApplication {
        initCut()
        everySuspend { handlerMock.upgradeRoom(any()) }.returns(UpgradeRoom.Response(RoomId("!nextRoom:server")))
        val response =
    ...
    fun shouldTimestampToEvent() = testApplication {
        ...
        val response =
            client.get("/_matrix/client/v1/rooms/!room:server/timestamp_to_event?ts=24&dir=f") { bearerAuth("token") }
        assertSoftly(response) {
            this.status shouldBe HttpStatusCode.OK
            this.contentType() shouldBe ContentType.Application.Json
            this.body<String>() shouldBe
