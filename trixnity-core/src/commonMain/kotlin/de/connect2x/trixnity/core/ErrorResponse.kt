...
sealed interface ErrorResponse {
    ...
    @Serializable @SerialName("M_FORBIDDEN") data class Forbidden(override val error: String) : ErrorResponse

    /**
     * The access token specified was not recognised.
     *
     * An additional response parameter, soft_logout, might be present on the response for 401 HTTP status codes. See
     * the [soft logout](https://matrix.org/docs/spec/client_server/latest#soft-logout) section for more information.
     */
    @Serializable
    @SerialName("M_UNKNOWN_TOKEN")
    data class UnknownToken(override val error: String, @SerialName("soft_logout") val softLogout: Boolean = false) :
        ErrorResponse

    /** The account has been suspended and can only be used for limited actions at this time. */
    @Serializable @SerialName("M_USER_SUSPENDED") data class UserSuspended(override val error: String) : ErrorResponse

    /** The account has been locked and cannot be used at this time. */
    @Serializable
    @SerialName("M_USER_LOCKED")
    data class UserLocked(override val error: String, @SerialName("soft_logout") val softLogout: Boolean = false) :
        ErrorResponse

    /** No access token was specified for the request. */
    @Serializable @SerialName("M_MISSING_TOKEN") data class MissingToken(override val error: String) : ErrorResponse

    /**
     * Request contained valid JSON, but it was malformed in some way, e.g. missing required keys, invalid values for
     * keys.
     */
    @Serializable @SerialName("M_BAD_JSON") data class BadJson(override val error: String) : ErrorResponse

    /** Request did not contain valid JSON. */
    @Serializable @SerialName("M_NOT_JSON") data class NotJson(override val error: String) : ErrorResponse

    /** No resource was found for this request. */
    @Serializable @SerialName("M_NOT_FOUND") data class NotFound(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_UNKNOWN") data class Unknown(override val error: String) : ErrorResponse

    /** The server did not understand the request. */
    @Serializable @SerialName("M_UNRECOGNIZED") data class Unrecognized(override val error: String) : ErrorResponse

    /** The request was not correctly authorized. Usually due to login failures. */
    @Serializable @SerialName("M_UNAUTHORIZED") data class Unauthorized(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_USER_IN_USE") data class UserInUse(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_ROOM_IN_USE") data class RoomInUse(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_THREEPID_IN_USE") data class ThirdPIdInUse(override val error: String) : ErrorResponse
     ...
     */
    @Serializable @SerialName("M_THREEPID_DENIED") data class ThirdPIdDenied(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_BAD_STATE") data class BadState(override val error: String) : ErrorResponse
    ...
    @Serializable @SerialName("M_CAPTCHA_NEEDED") data class CaptchaNeeded(override val error: String) : ErrorResponse

    /** The Captcha provided did not match what was expected. */
    @Serializable @SerialName("M_CAPTCHA_INVALID") data class CaptchaInvalid(override val error: String) : ErrorResponse

    /** A required parameter was missing from the request. */
    @Serializable @SerialName("M_MISSING_PARAM") data class MissingParam(override val error: String) : ErrorResponse

    /**
     * A parameter that was specified has the wrong value. For example, the server expected an integer and instead
     * received a string.
     */
    @Serializable @SerialName("M_INVALID_PARAM") data class InvalidParam(override val error: String) : ErrorResponse

    /** The request or entity was too large. */
    @Serializable @SerialName("M_TOO_LARGE") data class TooLarge(override val error: String) : ErrorResponse
     ...
     */
    @Serializable @SerialName("M_EXCLUSIVE") data class Exclusive(override val error: String) : ErrorResponse
     ...
     * Typically, this error will appear on routes which attempt to modify state (eg: sending messages, account data,
     * etc) and not routes which only read state (eg: `/sync`, get account data, etc).
     */
    @Serializable
     ...
     * [Server Notices](https://matrix.org/docs/spec/client_server/latest#server-notices) module for more information.
     */
    @Serializable
    ...
    /**
     * An application service used the `m.login.application_service` type without setting `inhibit_login` to `true`, but
     * the server doesn't support logging in via the Legacy authentication API.
     */
    @Serializable
    ...
    @Serializable @SerialName("M_INVITE_BLOCKED") data class InviteBlocked(override val error: String) : ErrorResponse

    /**
     * The request cannot be completed because the user has exceeded (or the request would cause them to exceed) a limit
    ...
    @Serializable
    data class CustomErrorResponse(@SerialName("errcode") val errorCode: String, override val error: String) :
        ErrorResponse
...
fun Json.decodeErrorResponse(body: JsonObject): ErrorResponse =
    try {
        ...
        ErrorResponse.BadJson("response could not be parsed to ErrorResponse (body=$body): ${error.message}")
    }
