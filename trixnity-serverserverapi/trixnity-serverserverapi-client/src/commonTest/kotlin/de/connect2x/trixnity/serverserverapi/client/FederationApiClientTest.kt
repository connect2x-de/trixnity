...
class FederationApiClientTest : TrixnityBaseTest() {
    ...
    fun shouldGetEventAuthChain() = runTest {
        ...
        matrixRestClient.federation
            .getEventAuthChain(Url(""), RoomId("!room:server"), EventId("$1event"))
            .getOrThrow() shouldBe GetEventAuthChain.Response(listOf(pdu))
    }
    ...
    fun shouldBackfillRoom() = runTest {
        ...
        matrixRestClient.federation
            .backfillRoom(Url(""), RoomId("!room:server"), listOf(EventId("$1event")), 10)
            .getOrThrow() shouldBe
            PduTransaction(origin = "matrix.org", originTimestamp = 1234567890, pdus = listOf(pdu))
    }
    ...
    fun shouldGetEvent() = runTest {
        ...
        matrixRestClient.federation.getEvent(Url(""), EventId("$1event")).getOrThrow() shouldBe
            PduTransaction(origin = "matrix.org", originTimestamp = 1234567890, pdus = listOf(pdu))
    }
    ...
    fun shouldGetState() = runTest {
        ...
        matrixRestClient.federation.getState(Url(""), RoomId("!room:server"), EventId("$1event")).getOrThrow() shouldBe
            GetState.Response(authChain = listOf(pdu), pdus = listOf(pdu))
    }
    ...
    fun shouldGetStateIds() = runTest {
        ...
        matrixRestClient.federation
            .getStateIds(Url(""), RoomId("!room:server"), EventId("$1event"))
            .getOrThrow() shouldBe
    ...
    fun shouldMakeJoin() = runTest {
        ...
        matrixRestClient.federation
            .makeJoin(Url(""), RoomId("!room:server"), UserId("@alice:example.com"), setOf("3"))
            .getOrThrow() shouldBe
            MakeJoin.Response(
                eventTemplate =
                    ...
                    Signed(
                        ...
                        mapOf(
                            ...
                            "resident.example.com" to
                                keysOf(Key.Ed25519Key("other_key_version", "a different signature")),
                        ),
                    ),
                state = listOf(),
            )
    ...
    fun shouldMakeKnock() = runTest {
        ...
        matrixRestClient.federation
            .makeKnock(Url(""), RoomId("!room:server"), UserId("@alice:example.com"), setOf("3"))
            .getOrThrow() shouldBe
            MakeKnock.Response(
                eventTemplate =
                    PersistentDataUnit.PersistentDataUnitV12.PersistentStateDataUnitV12(
    ...
    fun shouldMakeLeave() = runTest {
        ...
        matrixRestClient.federation
            .makeLeave(Url(""), RoomId("!room:server"), UserId("@alice:example.com"))
            .getOrThrow() shouldBe
            MakeLeave.Response(
                eventTemplate =
                    PersistentDataUnit.PersistentDataUnitV12.PersistentStateDataUnitV12(
        ...
        val matrixRestClient =
            MatrixServerServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/federation/v1/publicRooms", request.url.fullPath)
                            assertEquals(HttpMethod.Post, request.method)
                            assertEquals(
                                """
                            {
    ...
    fun shouldGetHierarchy() = runTest {
        ...
        matrixRestClient.federation.getHierarchy(Url(""), RoomId("!room:server"), true).getOrThrow() shouldBe
    ...
    fun shouldQueryDirectory() = runTest {
        ...
        matrixRestClient.federation.queryDirectory(Url(""), RoomAliasId("#alias:server")).getOrThrow() shouldBe
    ...
    fun shouldGetOIDCUserInfo() = runTest {
        ...
        matrixRestClient.federation.getOIDCUserInfo(Url(""), "token").getOrThrow() shouldBe
            GetOIDCUserInfo.Response(sub = UserId("@alice:example.com"))
    }
    ...
    fun shouldGetDevices() = runTest {
        val matrixRestClient =
            MatrixServerServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/federation/v1/user/devices/@alice:example.com", request.url.fullPath)
                            assertEquals(HttpMethod.Get, request.method)
                            respond(
                                """
                                {
        ...
        matrixRestClient.federation.getDevices(Url(""), UserId("@alice:example.com")).getOrThrow() shouldBe
            GetDevices.Response(
                devices =
                    setOf(
    ...
    fun shouldClaimKeys() = runTest {
        val matrixRestClient =
            MatrixServerServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/federation/v1/user/keys/claim", request.url.fullPath)
                            assertEquals(HttpMethod.Post, request.method)
                            assertEquals(
                                """
                            {
        ...
        matrixRestClient.federation
            .claimKeys(
                ...
                ClaimKeys.Request(
                    oneTimeKeys =
                        mapOf(UserId("@alice:example.com") to mapOf("JLAFKJWSCS" to KeyAlgorithm.SignedCurve25519))
                ),
            )
            .getOrThrow() shouldBe
            ClaimKeys.Response(
    ...
    fun shouldGetKeys() = runTest {
        val matrixRestClient =
            MatrixServerServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/federation/v1/user/keys/query", request.url.fullPath)
                            assertEquals(HttpMethod.Post, request.method)
                            assertEquals(
                                """
                            {
        ...
        matrixRestClient.federation
            .getKeys(Url(""), GetKeys.Request(keysFrom = mapOf(UserId("alice", "example.com") to setOf())))
            .getOrThrow() shouldBe
            GetKeys.Response(
                deviceKeys =
                    mapOf(
        ...
        val matrixRestClient =
            MatrixServerServerApiClientImpl(
                ...
                httpClientEngine =
                    scopedMockEngine {
                        addHandler { request ->
                            assertEquals("/_matrix/federation/v1/media/download/mediaId123", request.url.fullPath)
                            assertEquals(HttpMethod.Get, request.method)
                            respond(
                                """
                                --boundary
    ...
    fun shouldDownloadThumbnailStream() = runTest {
        ...
        matrixRestClient.federation
            .downloadThumbnail("mediaId123", 64, 64, SCALE) { media ->
                media.shouldBeInstanceOf<Media.Stream>()
                media.contentLength shouldBe 22
                media.contentType shouldBe ContentType.Text.Plain
                media.contentDisposition shouldBe
